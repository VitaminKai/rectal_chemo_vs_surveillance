...
>Comparing the baseline profiles between adjuvant chemo versus surveillance group.
Compared to the adjuvant chemotherapy group, patients in the surveillence group tends to be older and present with better prognostic profile. [Add in statistics from table] 
...
> No significant difference in overall survival was detected between the KM curve of two groups using log-rank test (p=0.58)
...
> There is a trend towards better recurrence free survival in the surveillance group using log-rank test however statistical significance was not reached (p=0.043 used to be not significant, awaiting confirmation on some seemingly wrong entries on raw excel). 
...
>Univariate cox regression was conducted to check each included variables influence on final survival outcome

### Overall survival

```{r cox analysis for os，setup,include=FALSE}
...
# Multivariate cox analysis for RFS
...
## Firth penalised multivariate cox regression

>Next we aimed to compare the OS and RFS between adjuvant chemo and surveillance group using multivariate logistical regression accounting for known clinical prognostic factors. Firth penalised cox regression was used as some variables contained groups with relatively small number of samples.A maximum iteration of 500 and maximum step per iteration of 0.5 was used. 

```{r, Firth penalised regression}

### Normal cox regression without firth penalisation
...
var_names <- var_names %>% str_replace("cancer_stage_yp", "frailty(cancer_stage_yp)")

for (i in 21:31) {
  print(i)
  clinical_multi_complete_df_2 <- slice(clinical_multi_complete_df, i:120)
  os_multi_surv_table <- coxph(as.formula(paste('Surv(time=os_time,event=os_status)~',var_names)), 
                                     data = clinical_multi_complete_df_2)
}

clinical_multi_complete_df %>% slice(27:29)

clinical_multi_complete_df_2 <- slice(clinical_multi_complete_df, c(1:27, 29:120))
...
pca_result <- clinical_multi_complete_df %>% select(-1) %>% 
...
# is it data's problem or test's problem: 
# can we use a subset of data 
...
fit.breast<-coxph(data=breast, Surv(TIME,CENS)~T+ridge(N+G+CD, theta = 1))

fit.breast %>% tbl_regression()

fit.breast %>% broom::glance()
